...
> **Не хотите заниматься сервером?** [fastoshop.by](https://fastoshop.by), поставим и будем следить: домен, сертификат, бэкапы, обновления, поддержка и перенос каталога из любого прайса, цена на [fastoshop.by](https://fastoshop.by/#price). Это наш платный хостинг; сам FastoShop был и остаётся бесплатным, ставьте на свой VPS и никому ничего не должны.

## Зачем второй канал

Продажа на площадке - это не та цена, что стоит на ценнике. Комиссия, логистика, хранение, возвраты и продвижение считаются от финальной цены, и на обычной карточке 15–25% уходит на одну лишь комиссию, до всего остального. Тот же заказ, принятый на своём сайте, несёт одно удержание - приём платежа.

Это арифметика. Две вещи важнее неё:

- **Покупатель принадлежит площадке, а не вам.** Написать повторно некому: вы не знаете, кто это был. Заявка со своего сайта приходит с именем и телефоном.
- **Правила тоже не ваши.** Акции, тарифы и лимиты меняются без вас. Канал, которым владеете вы, работает независимо от того, что решит кабинет в этом квартале.

Ничто из этого не довод уходить с площадки. Маркетплейс - место, где вас находят впервые. Смысл в том, что часть оборота, прежде всего повторные покупки, может приходить без посредника.
...
- **Страница под каждую категорию** - `/c/tekstil/spalnya/kpb-evro` со своим заголовком, описанием и хлебными крошками: запрос «купить КПБ евро оптом» ведёт на посадочную страницу, а не на общий каталог. Дерево приезжает из источника само, из YML-фида, таксономии Ozon, справочника предметов WB или ячейки прайса вида «Текстиль > Спальня». На живом каталоге одного оптовика это 570 посадочных страниц вместо одной.
...
- **Готовые фиды для рекламы, без выгрузок и плагинов** - `/yml.xml` для Яндекса и `/gmc.xml` для Google Merchant Center собираются из живого каталога и всегда свежие. Товарной кампании в Директе достаточно дать ссылку: в фиде уже есть артикул, цена, валюта, наличие, дерево категорий, картинка и описание по каждому предложению. На живом магазине это 23 844 предложения; поменяли цену - фид поменялся, перезаливать нечего.
- **AI-поиск - тот же контракт**: ассистенты (ChatGPT, Perplexity, Алиса) отвечают «где купить X» по страницам, которые могут прочитать, - серверный HTML с ценой в разметке не требует исполнения скриптов, а `/llms.txt`, собранный из живого каталога, отдаёт им магазин одним чтением: что продаётся, в каких разделах, как оформляется заказ.
...
- ✍️ **Текст карточки, переписанный моделью** - кнопка «Улучшить текст (AI)» в карточке товара превращает строку из накладной («Ерш унитазный с/подст "Шляпа" д13х36см пл ванильный») в название и описание, которые читает человек, и предлагает подходящий раздел из ваших же категорий. Модель работает только с фактами исходника: выдумывать свойства, комплектацию и материалы ей запрещено, а раздел она может только выбрать из существующих - придуманный магазин отбросит сам. Результат подставляется в форму как черновик: вы читаете, правите и сохраняете сами, и повторная загрузка прайса ваш текст уже не тронет. Услуга платная и оплачивается отдельно, по ключу [AdHunters](https://adhunters.fastolead.com) (пакеты от 5 BYN); сам FastoShop за это денег не берёт, и без ключа кнопки просто нет.
...
- Онлайн-оплаты и фискальных чеков. Заказ - это заявка: покупатель оставляет имя и телефон или почту, вам приходит письмо, сделка закрывается по телефону.
- Промокодов и акций со скидками.
- Вариантов товара: наборы и фасовки есть, размер и цвет пока отдельные позиции.
- Отзывов покупателей. Без них в выдаче нет звёзд, а рисовать рейтинг и запрещено, и бессмысленно.
- Условий доставки и возврата в виде полей: и то и другое живёт свободным текстом на страницах магазина, поэтому поисковик прочитать их не может.
- Остатков FBO - товар на складе площадки считает сама площадка.
- Мультиарендности: одна установка - один магазин и один владелец, намеренно.
- Мобильного приложения. Витрина адаптивная, приложения не будет.

И про сроки честно: трафик из поиска не приходит сразу. Новый сайт обходится неделями, а сколько страниц окажется в индексе, зависит от размера каталога и от того, что на страницах написано. Позиций в выдаче здесь никто не обещает.
...
**Требования:** Debian 12+ или Ubuntu 22.04+, amd64, домен с A-записью на этот сервер. Одного общего vCPU и 1 ГБ памяти хватает на каталог в 20 000 позиций; размер диска определяют фотографии, а не база - она остаётся в пределах десятков мегабайт. Более старые выпуски не поддерживаются.
...
Если домен уже публичный, визард лучше не оставлять открытым: `sudo -u fastocloud fastoshop -create-owner you@example.com` заводит владельца сразу и печатает сгенерированный пароль, после этого визард недоступен.

**Забыли пароль?** `sudo -u fastocloud fastoshop -reset-password` - напечатает новый и разлогинит все сессии. Почта для этого не нужна.

Обе команды запускаются от пользователя сервиса: база лежит в его домашнем каталоге, и от root получилась бы вторая, пустая.
...
│                                                                                    │
│  вкладки каналов: Ozon, Wildberries                                               │
└────────────────────────────────────────────────────────────────────────────────────┘
```
...
src/app/importer/     импорт каталога: Ozon, WB, YML-выгрузка, CSV, XLSX
...
- [x] Админка на двух языках (ru/en); витрина на русском
