...
const (
	...
	KeyOzonCurrencyMismatch = "ozon_currency_mismatch"
...
var kMessages = map[string][2]string{
	...
	KeyOzonCurrencyMismatch: {
		"валюта витрины и валюта кабинета Ozon не совпадают: цена площадки считается от цены витрины без пересчёта. Приведите их к одной валюте или проставьте цены вручную",
		"the storefront currency and the Ozon account currency differ: the platform price is derived from the shelf price without conversion. Set them to the same currency, or fill the prices by hand",
	},
	KeyOzonNoKeys: {
