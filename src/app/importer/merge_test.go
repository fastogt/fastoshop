...
func TestMergeUpdatesPriceAndStockOnly(t *testing.T) {
	...
	if _, err := Run(context.Background(), src, d, "Ромашка", 2, nil); err != nil {
		t.Fatal(err)
	}
	...
	res, err := Run(context.Background(), src, d, "Ромашка", 2, nil)
	if err != nil {
		t.Fatal(err)
	}
...
// Stopping an import must not withdraw the catalogue. A run that finishes takes
// everything the feed no longer lists off sale, which is right; a run that was
// stopped has simply not reached those rows yet, and zeroing them would empty a
// shop over a cancelled job.
func TestStoppedImportLeavesTheCatalogueOnSale(t *testing.T) {
	d := mergeDB(t)
	stocked := &feed{name: "yml", items: []Item{
		{SKU: "A", Title: "Первый", Price: 100, Stock: 5},
		{SKU: "B", Title: "Второй", Price: 200, Stock: 7},
	}}
	if _, err := Run(context.Background(), stocked, d, "Ромашка", 1, nil); err != nil {
		t.Fatal(err)
	}

	// A different feed: finishing this run would legitimately zero A and B.
	next := &feed{name: "yml", items: []Item{
		{SKU: "C", Title: "Третий", Price: 300, Stock: 3},
	}}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	res, err := Run(ctx, next, d, "Ромашка", 1, nil)
	if err != nil {
		t.Fatal(err)
	}
	if res.Zeroed != 0 {
		t.Errorf("a stopped import zeroed %d products", res.Zeroed)
	}
	if res.Imported != 0 {
		t.Errorf("a stopped import still wrote %d products", res.Imported)
	}
	...
	for _, p := range products {
		if p.Stock == 0 {
			t.Errorf("%s was taken off sale by a stopped import", p.SKU)
		}
	}
}
