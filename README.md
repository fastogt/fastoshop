...
| Google Search Console | [search.google.com/search-console](https://search.google.com/search-console) → «Ресурс с префиксом URL» → «Тег HTML» | То же самое в Google |
...
**Пока поля пустые, на витрине нет ни одного `<script>`** — это и есть обещание нулевого JavaScript. Каждый заполненный счётчик его нарушает: чужой код грузится на всех страницах, замедляет их и ставит куки, о которых по GDPR положено предупреждать. Метрика и GA одновременно — двойная плата за одно и то же; выбирайте один, если не нужны оба кабинета.

Что настройки **не** делают: не регистрируют сайт в поисковике за вас, не ускоряют индексацию и не заменяют карту сайта. Новому домену на попадание в выдачу нужны недели — это нормально и не чинится настройками.
