...
- **nginx is always in front of the binary; the Go process is never exposed directly.** Transport belongs to nginx: TLS, compression, client body limits, slow-client protection. `packaging/nginx-fastoshop.conf.template` is a sample for a standalone install, not a contract. The application does not terminate TLS and does not compress responses; `middleware.Compress(5)` in `cmd/fastoshop.go` is a leftover, removed only once the nginx in front compresses the same types. What stays in Go is content and its cache semantics - `ETag`, `Cache-Control`, `Last-Modified`.
