...
- **Storefront** (`app/storefront`) - server-rendered `html/template`, **zero JavaScript**. SEO is priority #1: JSON-LD `schema.org/Product`, sitemap.xml with lastmod, canonical, OG, human-readable slugs with transliteration. None of this may be broken "for convenience". AI search is part of the same contract: `/llms.txt` is generated per shop from settings and live categories (name, how ordering works, sections with counts) - answer engines already send buyers, and they read the shop's shape from this file instead of twenty thousand pages.
...
- **Channels are vertical slices, not a shared abstraction.** Marketplace sync is built as separate admin tabs (Ozon → Wildberries): each has its own Go package (`app/ozon`, `app/wb`), its own tables with the platform prefix (`ozon_settings`, `ozon_links`), its own rules. The core (`products`, `orders`) knows nothing about platforms. A shared interface, if one ever emerges, is derived from finished tabs, not designed ahead of them. After two finished tabs, what turned out to be shared is the markup-ladder arithmetic (`database/price_rules.go`) and the plumbing that carries no platform meaning - `app/channel`: page and filter parsing, retry delays, the wire structs both tabs answer with, the worker's wake-up signals; on the web side `PriceLadder` and `PublicationPanel`. Everything with platform semantics diverged: Ozon sets stock by `offer_id` and answers per line item immediately, WB sets stock by the size barcode and price by `nmID`, and returns the result as a task later. An interface designed from Ozon alone would have broken exactly there. A third channel copies only its own part - tables, client, sync - and takes the rest from `app/channel`.
- **Card enrichment is somebody else's service, not ours.** "Improve the text (AI)" proxies to AdHunters with the owner's own key (`app/handler/enrich.go`); the model, the prompt and the billing live there, and the shop keeps no key of its own - an owner without a key does not get the button. The draft is never written: it goes into the admin form, and the owner saves it themselves, because a generated text that saved itself would put invented properties on the storefront under their name. Two fields only, title and description, plus a category **picked from the shop's existing tree** and dropped unless it matches one exactly - guessing a hierarchy stays forbidden. Re-import must never take any of it back: `merge()` writes only price, stock and an empty category, and `TestReimportKeepsTheOwnersWords` holds that line.
...
- **HTTP responses only through gofastogt**: the `{"data": ...}` envelope and errors are assembled in one place - `app/httpjson` (`WriteOK` → `gofastogt.NewOkResponse`, `WriteBadRequest`/`WriteInternalError` → `errorgt.MakeErrorJson*`); handlers call them directly. No `json.Marshal` + `w.Write` in handlers.
...
- **Secrets never leave the server**: a password or a key in a JSON response is a boolean `*_set` flag, or the last four characters when the owner needs to tell two keys apart (`maskKey` in `app/handler/settings.go`) - never the value. The matching request field is a `*string`, so a body without it leaves the stored secret alone and the masked value can never be saved back as the key.
...
- **nginx is always in front of the binary; the Go process is never exposed directly.** Transport belongs to nginx: TLS, compression, client body limits, slow-client protection. `packaging/nginx-fastoshop.conf.template` is a sample for a standalone install, not a contract. The application does not terminate TLS and does not compress responses. What stays in Go is content and its cache semantics - `ETag`, `Cache-Control`, `Last-Modified`.
...
- **The CLI and its stdout have an external consumer - the private fastoshop-infra repo.** `fastoshopctl create` calls `fastoshop -invite-owner`, parses the `Invite (valid 24h): <url>` line with `sed`, and emails it - this is the platform's only onboarding path. The flags, `/api/invite`, and the output format look unused from inside this repository, but they must not break: before removing any flag or endpoint, or changing stdout, grep `fastoshop-infra/packaging/`. The same repo reads the database directly: `tools/funnel.py` and `tools/order_source.py` select from `products`, `orders`, `order_items` and `settings` and parse the `orders.source` format, so before renaming a column or changing that format, grep `fastoshop-infra/tools/`.
...
`README.md` - what the product is and how to install it. `CONTRIBUTING.md` - building, pre-PR checks, how to add your own channel or import source.
